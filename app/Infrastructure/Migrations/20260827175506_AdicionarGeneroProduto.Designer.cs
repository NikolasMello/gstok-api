// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using gstok_api.Database;

#nullable disable

namespace gstok_api.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260827175506_AdicionarGeneroProduto")]
    partial class AdicionarGeneroProduto
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("gstok_api.Models.CarrinhoItemModel", b =>
                {
                    b.Property<Guid>("IdCarrinhoItem")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_carrinho_item");

                    b.Property<Guid>("CarrinhoId")
                        .HasColumnType("uuid")
                        .HasColumnName("carrinho_id");

                    b.Property<Guid>("EstoqueId")
                        .HasColumnType("uuid")
                        .HasColumnName("estoque_id");

                    b.Property<int>("QtQuantidade")
                        .HasColumnType("integer")
                        .HasColumnName("qt_quantidade");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdCarrinhoItem");

                    b.HasIndex("EstoqueId");

                    b.HasIndex("CarrinhoId", "EstoqueId")
                        .IsUnique();

                    b.ToTable("carrinho_item");
                });

            modelBuilder.Entity("gstok_api.Models.CarrinhoModel", b =>
                {
                    b.Property<Guid>("IdCarrinho")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_carrinho");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uuid")
                        .HasColumnName("cliente_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdCarrinho");

                    b.HasIndex("ClienteId")
                        .IsUnique();

                    b.ToTable("carrinho");
                });

            modelBuilder.Entity("gstok_api.Models.ClienteModel", b =>
                {
                    b.Property<Guid>("IdCliente")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_cliente");

                    b.Property<Guid>("PessoaId")
                        .HasColumnType("uuid")
                        .HasColumnName("pessoa_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdCliente");

                    b.HasIndex("PessoaId")
                        .IsUnique();

                    b.ToTable("cliente");
                });

            modelBuilder.Entity("gstok_api.Models.ColecaoModel", b =>
                {
                    b.Property<Guid>("IdColecao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_colecao");

                    b.Property<Guid>("FornecedorId")
                        .HasColumnType("uuid")
                        .HasColumnName("fornecedor_id");

                    b.Property<string>("NmColecao")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_colecao");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdColecao");

                    b.HasIndex("FornecedorId", "NmColecao")
                        .IsUnique();

                    b.ToTable("colecao");
                });

            modelBuilder.Entity("gstok_api.Models.CompraItemModel", b =>
                {
                    b.Property<Guid>("IdItemCompra")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_item_compra");

                    b.Property<Guid>("CompraId")
                        .HasColumnType("uuid")
                        .HasColumnName("compra_id");

                    b.Property<Guid>("CorProdutoId")
                        .HasColumnType("uuid")
                        .HasColumnName("cor_produto_id");

                    b.Property<Guid?>("EstoqueId")
                        .HasColumnType("uuid")
                        .HasColumnName("estoque_id");

                    b.Property<int>("QtQuantidade")
                        .HasColumnType("integer")
                        .HasColumnName("qt_quantidade");

                    b.Property<string>("TpTamanho")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("tp_tamanho");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<decimal>("VlTotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total");

                    b.Property<decimal>("VlUnitario")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_unitario");

                    b.HasKey("IdItemCompra");

                    b.HasIndex("CompraId");

                    b.HasIndex("CorProdutoId");

                    b.HasIndex("EstoqueId");

                    b.ToTable("item_compra");
                });

            modelBuilder.Entity("gstok_api.Models.CompraModel", b =>
                {
                    b.Property<Guid>("IdCompra")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_compra");

                    b.Property<DateOnly>("DtCompra")
                        .HasColumnType("date")
                        .HasColumnName("dt_compra");

                    b.Property<DateOnly?>("DtPrevisaoEntrega")
                        .HasColumnType("date")
                        .HasColumnName("dt_previsao_entrega");

                    b.Property<DateOnly?>("DtRecebimento")
                        .HasColumnType("date")
                        .HasColumnName("dt_recebimento");

                    b.Property<Guid>("FornecedorId")
                        .HasColumnType("uuid")
                        .HasColumnName("fornecedor_id");

                    b.Property<string>("StCompra")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("st_compra");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<decimal>("VlTotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total");

                    b.HasKey("IdCompra");

                    b.HasIndex("FornecedorId");

                    b.ToTable("compra");
                });

            modelBuilder.Entity("gstok_api.Models.ContaClienteModel", b =>
                {
                    b.Property<Guid>("IdContaCliente")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_conta_cliente");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uuid")
                        .HasColumnName("cliente_id");

                    b.Property<string>("DsSenha")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ds_senha");

                    b.Property<string>("NmEmail")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("nm_email");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdContaCliente");

                    b.HasIndex("ClienteId")
                        .IsUnique();

                    b.HasIndex("NmEmail")
                        .IsUnique();

                    b.ToTable("conta_cliente");
                });

            modelBuilder.Entity("gstok_api.Models.CorProdutoModel", b =>
                {
                    b.Property<Guid>("IdCorProduto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_cor_produto");

                    b.Property<string>("CdHex")
                        .IsRequired()
                        .HasMaxLength(7)
                        .HasColumnType("character varying(7)")
                        .HasColumnName("cd_hex");

                    b.Property<string>("CdHex2")
                        .HasMaxLength(7)
                        .HasColumnType("character varying(7)")
                        .HasColumnName("cd_hex2");

                    b.Property<string>("CdHex3")
                        .HasMaxLength(7)
                        .HasColumnType("character varying(7)")
                        .HasColumnName("cd_hex3");

                    b.Property<string>("NmCor")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("nm_cor");

                    b.Property<Guid>("ProdutoId")
                        .HasColumnType("uuid")
                        .HasColumnName("produto_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdCorProduto");

                    b.HasIndex("ProdutoId", "NmCor")
                        .IsUnique();

                    b.ToTable("cor_produto");
                });

            modelBuilder.Entity("gstok_api.Models.DevolucaoItemModel", b =>
                {
                    b.Property<Guid>("IdItemDevolucao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_item_devolucao");

                    b.Property<Guid>("DevolucaoId")
                        .HasColumnType("uuid")
                        .HasColumnName("devolucao_id");

                    b.Property<int>("QtQuantidade")
                        .HasColumnType("integer")
                        .HasColumnName("qt_quantidade");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<Guid>("VendaItemId")
                        .HasColumnType("uuid")
                        .HasColumnName("venda_item_id");

                    b.Property<decimal>("VlTotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total");

                    b.Property<decimal>("VlUnitario")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_unitario");

                    b.HasKey("IdItemDevolucao");

                    b.HasIndex("DevolucaoId");

                    b.HasIndex("VendaItemId");

                    b.ToTable("item_devolucao");
                });

            modelBuilder.Entity("gstok_api.Models.DevolucaoModel", b =>
                {
                    b.Property<Guid>("IdDevolucao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_devolucao");

                    b.Property<string>("DsMotivo")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("ds_motivo");

                    b.Property<string>("StDevolucao")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("st_devolucao");

                    b.Property<string>("TpReembolso")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("tp_reembolso");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<Guid>("VendaId")
                        .HasColumnType("uuid")
                        .HasColumnName("venda_id");

                    b.Property<decimal>("VlTotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total");

                    b.HasKey("IdDevolucao");

                    b.HasIndex("VendaId");

                    b.ToTable("devolucao");
                });

            modelBuilder.Entity("gstok_api.Models.EnderecoModel", b =>
                {
                    b.Property<Guid>("IdEndereco")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_endereco");

                    b.Property<string>("CdCep")
                        .IsRequired()
                        .HasMaxLength(9)
                        .HasColumnType("character varying(9)")
                        .HasColumnName("cd_cep");

                    b.Property<string>("CdNumero")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("cd_numero");

                    b.Property<string>("CdUf")
                        .IsRequired()
                        .HasMaxLength(2)
                        .HasColumnType("character varying(2)")
                        .HasColumnName("cd_uf");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uuid")
                        .HasColumnName("cliente_id");

                    b.Property<string>("DsComplemento")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("ds_complemento");

                    b.Property<bool>("FlPrincipal")
                        .HasColumnType("boolean")
                        .HasColumnName("fl_principal");

                    b.Property<string>("NmBairro")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_bairro");

                    b.Property<string>("NmCidade")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_cidade");

                    b.Property<string>("NmLogradouro")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("nm_logradouro");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdEndereco");

                    b.HasIndex("ClienteId")
                        .IsUnique()
                        .HasFilter("fl_principal = true");

                    b.ToTable("endereco");
                });

            modelBuilder.Entity("gstok_api.Models.EstoqueModel", b =>
                {
                    b.Property<Guid>("IdEstoque")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_estoque");

                    b.Property<Guid>("CorProdutoId")
                        .HasColumnType("uuid")
                        .HasColumnName("cor_produto_id");

                    b.Property<int>("QtEstoque")
                        .HasColumnType("integer")
                        .HasColumnName("qt_estoque");

                    b.Property<string>("TpTamanho")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("tp_tamanho");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdEstoque");

                    b.HasIndex("CorProdutoId", "TpTamanho")
                        .IsUnique();

                    b.ToTable("estoque");
                });

            modelBuilder.Entity("gstok_api.Models.FornecedorModel", b =>
                {
                    b.Property<Guid>("IdFornecedor")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_fornecedor");

                    b.Property<string>("CdCnpj")
                        .IsRequired()
                        .HasMaxLength(14)
                        .HasColumnType("character varying(14)")
                        .HasColumnName("cd_cnpj");

                    b.Property<string>("NmEmpresa")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("nm_empresa");

                    b.Property<string>("NmFantasia")
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("nm_fantasia");

                    b.Property<string>("NmMarca")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_marca");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdFornecedor");

                    b.HasIndex("CdCnpj")
                        .IsUnique();

                    b.ToTable("fornecedor");
                });

            modelBuilder.Entity("gstok_api.Models.FotoPessoaModel", b =>
                {
                    b.Property<Guid>("IdFotoPessoa")
                        .HasColumnType("uuid")
                        .HasColumnName("id_foto_pessoa");

                    b.Property<string>("NmImagem")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("nm_imagem");

                    b.Property<int>("NrAltura")
                        .HasColumnType("integer")
                        .HasColumnName("nr_altura");

                    b.Property<int>("NrLargura")
                        .HasColumnType("integer")
                        .HasColumnName("nr_largura");

                    b.Property<Guid>("PessoaId")
                        .HasColumnType("uuid")
                        .HasColumnName("pessoa_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<string>("UrImagem")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ur_imagem");

                    b.HasKey("IdFotoPessoa");

                    b.HasIndex("PessoaId")
                        .IsUnique();

                    b.ToTable("foto_pessoa");
                });

            modelBuilder.Entity("gstok_api.Models.ImagemProdutoModel", b =>
                {
                    b.Property<Guid>("IdImagemProduto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_imagem_produto");

                    b.Property<bool>("FlPrincipal")
                        .HasColumnType("boolean")
                        .HasColumnName("fl_principal");

                    b.Property<string>("NmCaption")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("nm_caption");

                    b.Property<int>("NrAlturaAvatar")
                        .HasColumnType("integer")
                        .HasColumnName("nr_altura_avatar");

                    b.Property<int>("NrAlturaDesktop")
                        .HasColumnType("integer")
                        .HasColumnName("nr_altura_desktop");

                    b.Property<int>("NrAlturaMobile")
                        .HasColumnType("integer")
                        .HasColumnName("nr_altura_mobile");

                    b.Property<int>("NrAlturaTablet")
                        .HasColumnType("integer")
                        .HasColumnName("nr_altura_tablet");

                    b.Property<int>("NrAlturaThumbnail")
                        .HasColumnType("integer")
                        .HasColumnName("nr_altura_thumbnail");

                    b.Property<int>("NrLarguraAvatar")
                        .HasColumnType("integer")
                        .HasColumnName("nr_largura_avatar");

                    b.Property<int>("NrLarguraDesktop")
                        .HasColumnType("integer")
                        .HasColumnName("nr_largura_desktop");

                    b.Property<int>("NrLarguraMobile")
                        .HasColumnType("integer")
                        .HasColumnName("nr_largura_mobile");

                    b.Property<int>("NrLarguraTablet")
                        .HasColumnType("integer")
                        .HasColumnName("nr_largura_tablet");

                    b.Property<int>("NrLarguraThumbnail")
                        .HasColumnType("integer")
                        .HasColumnName("nr_largura_thumbnail");

                    b.Property<Guid>("ProdutoId")
                        .HasColumnType("uuid")
                        .HasColumnName("produto_id");

                    b.Property<int>("SqOrdem")
                        .HasColumnType("integer")
                        .HasColumnName("sq_ordem");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<string>("UrAvatar")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ur_avatar");

                    b.Property<string>("UrDesktop")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ur_desktop");

                    b.Property<string>("UrMobile")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ur_mobile");

                    b.Property<string>("UrTablet")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ur_tablet");

                    b.Property<string>("UrThumbnail")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ur_thumbnail");

                    b.HasKey("IdImagemProduto");

                    b.HasIndex("ProdutoId");

                    b.ToTable("imagem_produto");
                });

            modelBuilder.Entity("gstok_api.Models.PessoaModel", b =>
                {
                    b.Property<Guid>("IdPessoa")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_pessoa");

                    b.Property<string>("CdInscricaoNacional")
                        .IsRequired()
                        .HasMaxLength(14)
                        .HasColumnType("character varying(14)")
                        .HasColumnName("cd_inscricao_nacional");

                    b.Property<string>("NmEmailContato")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("nm_email_contato");

                    b.Property<string>("NmPessoa")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_pessoa");

                    b.Property<string>("NmSobrenome")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_sobrenome");

                    b.Property<string>("NmTelefone")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("nm_telefone");

                    b.Property<string>("TpPessoa")
                        .IsRequired()
                        .HasMaxLength(1)
                        .HasColumnType("character varying(1)")
                        .HasColumnName("tp_pessoa");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdPessoa");

                    b.HasIndex("CdInscricaoNacional")
                        .IsUnique();

                    b.ToTable("pessoa");
                });

            modelBuilder.Entity("gstok_api.Models.ProdutoModel", b =>
                {
                    b.Property<Guid>("IdProduto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_produto");

                    b.Property<string>("CdEan")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("character varying(13)")
                        .HasColumnName("cd_ean");

                    b.Property<Guid>("ColecaoId")
                        .HasColumnType("uuid")
                        .HasColumnName("colecao_id");

                    b.Property<string>("DsProduto")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("ds_produto");

                    b.Property<bool>("FlAtivo")
                        .HasColumnType("boolean")
                        .HasColumnName("fl_ativo");

                    b.Property<string>("NmProduto")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("nm_produto");

                    b.Property<Guid>("TipoProdutoId")
                        .HasColumnType("uuid")
                        .HasColumnName("tipo_produto_id");

                    b.Property<string>("TpEstacao")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("tp_estacao");

                    b.Property<string>("TpGenero")
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("tp_genero");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<decimal>("VlPreco")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_preco");

                    b.Property<decimal>("VlVenda")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_venda");

                    b.HasKey("IdProduto");

                    b.HasIndex("ColecaoId");

                    b.HasIndex("TipoProdutoId");

                    b.ToTable("produto");
                });

            modelBuilder.Entity("gstok_api.Models.PromocaoModel", b =>
                {
                    b.Property<Guid>("IdPromocao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_promocao");

                    b.Property<DateOnly>("DtInicio")
                        .HasColumnType("date")
                        .HasColumnName("dt_inicio");

                    b.Property<DateOnly>("DtTermino")
                        .HasColumnType("date")
                        .HasColumnName("dt_termino");

                    b.Property<bool>("FlAtivo")
                        .HasColumnType("boolean")
                        .HasColumnName("fl_ativo");

                    b.Property<string>("NmPromocao")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_promocao");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdPromocao");

                    b.ToTable("promocao");
                });

            modelBuilder.Entity("gstok_api.Models.PromocaoProdutoModel", b =>
                {
                    b.Property<Guid>("IdPromocaoProduto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_promocao_produto");

                    b.Property<decimal>("PcDesconto")
                        .HasColumnType("numeric")
                        .HasColumnName("pc_desconto");

                    b.Property<Guid>("ProdutoId")
                        .HasColumnType("uuid")
                        .HasColumnName("produto_id");

                    b.Property<Guid>("PromocaoId")
                        .HasColumnType("uuid")
                        .HasColumnName("promocao_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdPromocaoProduto");

                    b.HasIndex("ProdutoId");

                    b.HasIndex("PromocaoId", "ProdutoId")
                        .IsUnique();

                    b.ToTable("promocao_produto");
                });

            modelBuilder.Entity("gstok_api.Models.SessaoClienteModel", b =>
                {
                    b.Property<Guid>("IdSessaoCliente")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_sessao_cliente");

                    b.Property<string>("CdToken")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("cd_token");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uuid")
                        .HasColumnName("cliente_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime>("TsExpiracao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_expiracao");

                    b.HasKey("IdSessaoCliente");

                    b.HasIndex("CdToken")
                        .IsUnique();

                    b.HasIndex("ClienteId");

                    b.ToTable("sessao_cliente");
                });

            modelBuilder.Entity("gstok_api.Models.SessaoModel", b =>
                {
                    b.Property<Guid>("IdSessao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_sessao");

                    b.Property<string>("CdToken")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("cd_token");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime>("TsExpiracao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_expiracao");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("usuario_id");

                    b.HasKey("IdSessao");

                    b.HasIndex("CdToken")
                        .IsUnique();

                    b.HasIndex("UsuarioId");

                    b.ToTable("sessao");
                });

            modelBuilder.Entity("gstok_api.Models.TipoProdutoModel", b =>
                {
                    b.Property<Guid>("IdTipoProduto")
                        .HasColumnType("uuid")
                        .HasColumnName("id_tipo_produto");

                    b.Property<string>("NmTipo")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_tipo");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdTipoProduto");

                    b.HasIndex("NmTipo")
                        .IsUnique();

                    b.ToTable("tipo_produto");
                });

            modelBuilder.Entity("gstok_api.Models.TrocaItemEntradaModel", b =>
                {
                    b.Property<Guid>("IdItemTrocaEntrada")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_item_troca_entrada");

                    b.Property<Guid>("EstoqueId")
                        .HasColumnType("uuid")
                        .HasColumnName("estoque_id");

                    b.Property<int>("QtQuantidade")
                        .HasColumnType("integer")
                        .HasColumnName("qt_quantidade");

                    b.Property<Guid>("TrocaId")
                        .HasColumnType("uuid")
                        .HasColumnName("troca_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<decimal>("VlTotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total");

                    b.Property<decimal>("VlUnitario")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_unitario");

                    b.HasKey("IdItemTrocaEntrada");

                    b.HasIndex("EstoqueId");

                    b.HasIndex("TrocaId");

                    b.ToTable("item_troca_entrada");
                });

            modelBuilder.Entity("gstok_api.Models.TrocaItemSaidaModel", b =>
                {
                    b.Property<Guid>("IdItemTrocaSaida")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_item_troca_saida");

                    b.Property<int>("QtQuantidade")
                        .HasColumnType("integer")
                        .HasColumnName("qt_quantidade");

                    b.Property<Guid>("TrocaId")
                        .HasColumnType("uuid")
                        .HasColumnName("troca_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<Guid>("VendaItemId")
                        .HasColumnType("uuid")
                        .HasColumnName("venda_item_id");

                    b.Property<decimal>("VlTotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total");

                    b.Property<decimal>("VlUnitario")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_unitario");

                    b.HasKey("IdItemTrocaSaida");

                    b.HasIndex("TrocaId");

                    b.HasIndex("VendaItemId");

                    b.ToTable("item_troca_saida");
                });

            modelBuilder.Entity("gstok_api.Models.TrocaModel", b =>
                {
                    b.Property<Guid>("IdTroca")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_troca");

                    b.Property<string>("DsMotivo")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("ds_motivo");

                    b.Property<string>("StTroca")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("st_troca");

                    b.Property<string>("TpPagamento")
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("tp_pagamento");

                    b.Property<string>("TpReembolso")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("tp_reembolso");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<Guid>("VendaId")
                        .HasColumnType("uuid")
                        .HasColumnName("venda_id");

                    b.Property<decimal>("VlDiferenca")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_diferenca");

                    b.Property<decimal>("VlTotalEntrada")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total_entrada");

                    b.Property<decimal>("VlTotalSaida")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total_saida");

                    b.HasKey("IdTroca");

                    b.HasIndex("VendaId");

                    b.ToTable("troca");
                });

            modelBuilder.Entity("gstok_api.Models.UsuarioModel", b =>
                {
                    b.Property<Guid>("IdUsuario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_usuario");

                    b.Property<string>("DsSenha")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ds_senha");

                    b.Property<string>("NmEmail")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("nm_email");

                    b.Property<string>("NmPessoa")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("nm_pessoa");

                    b.Property<Guid?>("PessoaId")
                        .HasColumnType("uuid")
                        .HasColumnName("pessoa_id");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.HasKey("IdUsuario");

                    b.HasIndex("NmEmail")
                        .IsUnique();

                    b.HasIndex("PessoaId")
                        .IsUnique()
                        .HasFilter("pessoa_id IS NOT NULL");

                    b.ToTable("usuario");
                });

            modelBuilder.Entity("gstok_api.Models.VendaItemModel", b =>
                {
                    b.Property<Guid>("IdItemVenda")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_item_venda");

                    b.Property<Guid>("EstoqueId")
                        .HasColumnType("uuid")
                        .HasColumnName("estoque_id");

                    b.Property<int>("QtQuantidade")
                        .HasColumnType("integer")
                        .HasColumnName("qt_quantidade");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<Guid>("VendaId")
                        .HasColumnType("uuid")
                        .HasColumnName("venda_id");

                    b.Property<decimal>("VlTotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total");

                    b.Property<decimal>("VlUnitario")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_unitario");

                    b.HasKey("IdItemVenda");

                    b.HasIndex("EstoqueId");

                    b.HasIndex("VendaId");

                    b.ToTable("item_venda");
                });

            modelBuilder.Entity("gstok_api.Models.VendaModel", b =>
                {
                    b.Property<Guid>("IdVenda")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id_venda");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uuid")
                        .HasColumnName("cliente_id");

                    b.Property<Guid?>("EnderecoEntregaId")
                        .HasColumnType("uuid")
                        .HasColumnName("endereco_entrega_id");

                    b.Property<string>("StPagamento")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("st_pagamento");

                    b.Property<string>("StVenda")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("st_venda");

                    b.Property<string>("TpOrigem")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasDefaultValue("Loja")
                        .HasColumnName("tp_origem");

                    b.Property<string>("TpPagamento")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("tp_pagamento");

                    b.Property<DateTime>("TsCriacao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_criacao");

                    b.Property<DateTime?>("TsEdicao")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ts_edicao");

                    b.Property<decimal>("VlDesconto")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_desconto");

                    b.Property<decimal>("VlFrete")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_frete");

                    b.Property<decimal>("VlSubtotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_subtotal");

                    b.Property<decimal>("VlTotal")
                        .HasColumnType("numeric")
                        .HasColumnName("vl_total");

                    b.HasKey("IdVenda");

                    b.HasIndex("ClienteId");

                    b.HasIndex("EnderecoEntregaId");

                    b.ToTable("venda");
                });

            modelBuilder.Entity("gstok_api.Models.CarrinhoItemModel", b =>
                {
                    b.HasOne("gstok_api.Models.CarrinhoModel", "Carrinho")
                        .WithMany("Itens")
                        .HasForeignKey("CarrinhoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.EstoqueModel", "Estoque")
                        .WithMany()
                        .HasForeignKey("EstoqueId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Carrinho");

                    b.Navigation("Estoque");
                });

            modelBuilder.Entity("gstok_api.Models.CarrinhoModel", b =>
                {
                    b.HasOne("gstok_api.Models.ClienteModel", "Cliente")
                        .WithOne("Carrinho")
                        .HasForeignKey("gstok_api.Models.CarrinhoModel", "ClienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Cliente");
                });

            modelBuilder.Entity("gstok_api.Models.ClienteModel", b =>
                {
                    b.HasOne("gstok_api.Models.PessoaModel", "Pessoa")
                        .WithOne("Cliente")
                        .HasForeignKey("gstok_api.Models.ClienteModel", "PessoaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Pessoa");
                });

            modelBuilder.Entity("gstok_api.Models.ColecaoModel", b =>
                {
                    b.HasOne("gstok_api.Models.FornecedorModel", "Fornecedor")
                        .WithMany("Colecoes")
                        .HasForeignKey("FornecedorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Fornecedor");
                });

            modelBuilder.Entity("gstok_api.Models.CompraItemModel", b =>
                {
                    b.HasOne("gstok_api.Models.CompraModel", "Compra")
                        .WithMany("Itens")
                        .HasForeignKey("CompraId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.CorProdutoModel", "CorProduto")
                        .WithMany()
                        .HasForeignKey("CorProdutoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.EstoqueModel", "Estoque")
                        .WithMany()
                        .HasForeignKey("EstoqueId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Compra");

                    b.Navigation("CorProduto");

                    b.Navigation("Estoque");
                });

            modelBuilder.Entity("gstok_api.Models.CompraModel", b =>
                {
                    b.HasOne("gstok_api.Models.FornecedorModel", "Fornecedor")
                        .WithMany("Compras")
                        .HasForeignKey("FornecedorId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Fornecedor");
                });

            modelBuilder.Entity("gstok_api.Models.ContaClienteModel", b =>
                {
                    b.HasOne("gstok_api.Models.ClienteModel", "Cliente")
                        .WithOne("ContaCliente")
                        .HasForeignKey("gstok_api.Models.ContaClienteModel", "ClienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Cliente");
                });

            modelBuilder.Entity("gstok_api.Models.CorProdutoModel", b =>
                {
                    b.HasOne("gstok_api.Models.ProdutoModel", "Produto")
                        .WithMany("CoresProduto")
                        .HasForeignKey("ProdutoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Produto");
                });

            modelBuilder.Entity("gstok_api.Models.DevolucaoItemModel", b =>
                {
                    b.HasOne("gstok_api.Models.DevolucaoModel", "Devolucao")
                        .WithMany("Itens")
                        .HasForeignKey("DevolucaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.VendaItemModel", "VendaItem")
                        .WithMany()
                        .HasForeignKey("VendaItemId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Devolucao");

                    b.Navigation("VendaItem");
                });

            modelBuilder.Entity("gstok_api.Models.DevolucaoModel", b =>
                {
                    b.HasOne("gstok_api.Models.VendaModel", "Venda")
                        .WithMany()
                        .HasForeignKey("VendaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Venda");
                });

            modelBuilder.Entity("gstok_api.Models.EnderecoModel", b =>
                {
                    b.HasOne("gstok_api.Models.ClienteModel", "Cliente")
                        .WithMany("Enderecos")
                        .HasForeignKey("ClienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Cliente");
                });

            modelBuilder.Entity("gstok_api.Models.EstoqueModel", b =>
                {
                    b.HasOne("gstok_api.Models.CorProdutoModel", "CorProduto")
                        .WithMany()
                        .HasForeignKey("CorProdutoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("CorProduto");
                });

            modelBuilder.Entity("gstok_api.Models.FotoPessoaModel", b =>
                {
                    b.HasOne("gstok_api.Models.PessoaModel", "Pessoa")
                        .WithOne("Foto")
                        .HasForeignKey("gstok_api.Models.FotoPessoaModel", "PessoaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Pessoa");
                });

            modelBuilder.Entity("gstok_api.Models.ImagemProdutoModel", b =>
                {
                    b.HasOne("gstok_api.Models.ProdutoModel", "Produto")
                        .WithMany("Imagens")
                        .HasForeignKey("ProdutoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Produto");
                });

            modelBuilder.Entity("gstok_api.Models.ProdutoModel", b =>
                {
                    b.HasOne("gstok_api.Models.ColecaoModel", "Colecao")
                        .WithMany("Produtos")
                        .HasForeignKey("ColecaoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.TipoProdutoModel", "TipoProduto")
                        .WithMany("Produtos")
                        .HasForeignKey("TipoProdutoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Colecao");

                    b.Navigation("TipoProduto");
                });

            modelBuilder.Entity("gstok_api.Models.PromocaoProdutoModel", b =>
                {
                    b.HasOne("gstok_api.Models.ProdutoModel", "Produto")
                        .WithMany("Promocoes")
                        .HasForeignKey("ProdutoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.PromocaoModel", "Promocao")
                        .WithMany("Produtos")
                        .HasForeignKey("PromocaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Produto");

                    b.Navigation("Promocao");
                });

            modelBuilder.Entity("gstok_api.Models.SessaoClienteModel", b =>
                {
                    b.HasOne("gstok_api.Models.ClienteModel", "Cliente")
                        .WithMany()
                        .HasForeignKey("ClienteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Cliente");
                });

            modelBuilder.Entity("gstok_api.Models.SessaoModel", b =>
                {
                    b.HasOne("gstok_api.Models.UsuarioModel", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("gstok_api.Models.TrocaItemEntradaModel", b =>
                {
                    b.HasOne("gstok_api.Models.EstoqueModel", "Estoque")
                        .WithMany()
                        .HasForeignKey("EstoqueId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.TrocaModel", "Troca")
                        .WithMany("ItensEntrada")
                        .HasForeignKey("TrocaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Estoque");

                    b.Navigation("Troca");
                });

            modelBuilder.Entity("gstok_api.Models.TrocaItemSaidaModel", b =>
                {
                    b.HasOne("gstok_api.Models.TrocaModel", "Troca")
                        .WithMany("ItensSaida")
                        .HasForeignKey("TrocaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.VendaItemModel", "VendaItem")
                        .WithMany()
                        .HasForeignKey("VendaItemId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Troca");

                    b.Navigation("VendaItem");
                });

            modelBuilder.Entity("gstok_api.Models.TrocaModel", b =>
                {
                    b.HasOne("gstok_api.Models.VendaModel", "Venda")
                        .WithMany()
                        .HasForeignKey("VendaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Venda");
                });

            modelBuilder.Entity("gstok_api.Models.UsuarioModel", b =>
                {
                    b.HasOne("gstok_api.Models.PessoaModel", "Pessoa")
                        .WithOne()
                        .HasForeignKey("gstok_api.Models.UsuarioModel", "PessoaId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Pessoa");
                });

            modelBuilder.Entity("gstok_api.Models.VendaItemModel", b =>
                {
                    b.HasOne("gstok_api.Models.EstoqueModel", "Estoque")
                        .WithMany()
                        .HasForeignKey("EstoqueId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.VendaModel", "Venda")
                        .WithMany("Itens")
                        .HasForeignKey("VendaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Estoque");

                    b.Navigation("Venda");
                });

            modelBuilder.Entity("gstok_api.Models.VendaModel", b =>
                {
                    b.HasOne("gstok_api.Models.ClienteModel", "Cliente")
                        .WithMany("Compras")
                        .HasForeignKey("ClienteId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("gstok_api.Models.EnderecoModel", "EnderecoEntrega")
                        .WithMany()
                        .HasForeignKey("EnderecoEntregaId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Cliente");

                    b.Navigation("EnderecoEntrega");
                });

            modelBuilder.Entity("gstok_api.Models.CarrinhoModel", b =>
                {
                    b.Navigation("Itens");
                });

            modelBuilder.Entity("gstok_api.Models.ClienteModel", b =>
                {
                    b.Navigation("Carrinho");

                    b.Navigation("Compras");

                    b.Navigation("ContaCliente");

                    b.Navigation("Enderecos");
                });

            modelBuilder.Entity("gstok_api.Models.ColecaoModel", b =>
                {
                    b.Navigation("Produtos");
                });

            modelBuilder.Entity("gstok_api.Models.CompraModel", b =>
                {
                    b.Navigation("Itens");
                });

            modelBuilder.Entity("gstok_api.Models.DevolucaoModel", b =>
                {
                    b.Navigation("Itens");
                });

            modelBuilder.Entity("gstok_api.Models.FornecedorModel", b =>
                {
                    b.Navigation("Colecoes");

                    b.Navigation("Compras");
                });

            modelBuilder.Entity("gstok_api.Models.PessoaModel", b =>
                {
                    b.Navigation("Cliente");

                    b.Navigation("Foto");
                });

            modelBuilder.Entity("gstok_api.Models.ProdutoModel", b =>
                {
                    b.Navigation("CoresProduto");

                    b.Navigation("Imagens");

                    b.Navigation("Promocoes");
                });

            modelBuilder.Entity("gstok_api.Models.PromocaoModel", b =>
                {
                    b.Navigation("Produtos");
                });

            modelBuilder.Entity("gstok_api.Models.TipoProdutoModel", b =>
                {
                    b.Navigation("Produtos");
                });

            modelBuilder.Entity("gstok_api.Models.TrocaModel", b =>
                {
                    b.Navigation("ItensEntrada");

                    b.Navigation("ItensSaida");
                });

            modelBuilder.Entity("gstok_api.Models.VendaModel", b =>
                {
                    b.Navigation("Itens");
                });
#pragma warning restore 612, 618
        }
    }
}
